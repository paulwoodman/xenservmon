...
class PortManager(object):
    ...
    def check_modem_lines(self, force_notification=False):
        modemstate = (
            (self.getCTS() and MODEMSTATE_MASK_CTS) |
            (self.getDSR() and MODEMSTATE_MASK_DSR) |
            (self.getRI() and MODEMSTATE_MASK_RI) |
            (self.getCD() and MODEMSTATE_MASK_CD)
        )
    ...
    def _telnetProcessSubnegotiation(self, suboption):
        ...
        if suboption[0:1] == COM_PORT_OPTION:
            ...
            if suboption[1:2] == SET_BAUDRATE:
                backup = self.baudrate
                try:
                    (self.baudrate,) = struct.unpack("!I", suboption[2:6])
                except ValueError, e:
                    if self.logger:
                        self.logger.error("failed to set baud rate: %s" % (e,))
                    self.baudrate = backup
                else:
                    if self.logger:
                        self.logger.info("changed baud rate: %s" % (self.baudrate,))
                self.rfc2217SendSubnegotiation(SERVER_SET_BAUDRATE, struct.pack("!I", self.baudrate))
            elif suboption[1:2] == SET_DATASIZE:
                backup = self.bytesize
                try:
                    (self.bytesize,) = struct.unpack("!B", suboption[2:3])
                except ValueError, e:
                    if self.logger:
                        self.logger.error("failed to set data size: %s" % (e,))
                    self.bytesize = backup
                else:
                    if self.logger:
                        self.logger.info("changed data size: %s" % (self.bytesize,))
                self.rfc2217SendSubnegotiation(SERVER_SET_DATASIZE, struct.pack("!B", self.bytesize))
            elif suboption[1:2] == SET_PARITY:
                backup = self.parity
                try:
                    self.parity = RFC2217_REVERSE_PARITY_MAP[struct.unpack("!B", suboption[2:3])[0]]
                except ValueError, e:
                    if self.logger:
                    ...
                    if self.logger:
                        self.logger.info("changed parity: %s" % (self.parity,))
                self.rfc2217SendSubnegotiation(
                    SERVER_SET_PARITY,
                    struct.pack("!B", RFC2217_PARITY_MAP[self.parity])
                    )
            elif suboption[1:2] == SET_STOPSIZE:
                backup = self.stopbits
                try:
                    self.stopbits = RFC2217_REVERSE_STOPBIT_MAP[struct.unpack("!B", suboption[2:3])[0]]
                except ValueError, e:
                    if self.logger:
                        self.logger.error("failed to set stop bits: %s" % (e,))
                    self.stopbits = backup
                else:
                    if self.logger:
                        self.logger.info("changed stop bits: %s" % (self.stopbits,))
                self.rfc2217SendSubnegotiation(
                    SERVER_SET_STOPSIZE,
